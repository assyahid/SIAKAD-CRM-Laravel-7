...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.listJadwalPelajaran.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.list-jadwal-pelajarans.update", [$listJadwalPelajaran->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tahun_ajaran') ? 'is-invalid' : '' }}" name="tahun_ajaran_id" id="tahun_ajaran_id" required>
                    @foreach($tahun_ajarans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tahun_ajaran_id') ? old('tahun_ajaran_id') : $listJadwalPelajaran->tahun_ajaran->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jurusan') ? 'is-invalid' : '' }}" name="jurusan_id" id="jurusan_id" required>
                    @foreach($jurusans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('jurusan_id') ? old('jurusan_id') : $listJadwalPelajaran->jurusan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pelajaran') ? 'is-invalid' : '' }}" name="pelajaran_id" id="pelajaran_id" required>
                    @foreach($pelajarans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pelajaran_id') ? old('pelajaran_id') : $listJadwalPelajaran->pelajaran->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('dari_jam') ? 'is-invalid' : '' }}" type="text" name="dari_jam" id="dari_jam" value="{{ old('dari_jam', $listJadwalPelajaran->dari_jam) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('sampai_jam') ? 'is-invalid' : '' }}" type="text" name="sampai_jam" id="sampai_jam" value="{{ old('sampai_jam', $listJadwalPelajaran->sampai_jam) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guru') ? 'is-invalid' : '' }}" name="guru_id" id="guru_id" required>
                    @foreach($gurus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guru_id') ? old('guru_id') : $listJadwalPelajaran->guru->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('guru'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kelas') ? 'is-invalid' : '' }}" name="kelas_id" id="kelas_id" required>
                    @foreach($kelas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kelas_id') ? old('kelas_id') : $listJadwalPelajaran->kelas->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('kelas'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id" required>
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $listJadwalPelajaran->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
