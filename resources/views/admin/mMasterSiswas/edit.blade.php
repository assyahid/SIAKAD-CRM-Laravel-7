...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.mMasterSiswa.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.m-master-siswas.update", [$mMasterSiswa->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.mMasterSiswa.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $mMasterSiswa->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_lahir') ? 'is-invalid' : '' }}" type="text" name="tgl_lahir" id="tgl_lahir" value="{{ old('tgl_lahir', $mMasterSiswa->tgl_lahir) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nisn') ? 'is-invalid' : '' }}" type="text" name="nisn" id="nisn" value="{{ old('nisn', $mMasterSiswa->nisn) }}" required>
                @if($errors->has('nisn'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nisn') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('angkatan') ? 'is-invalid' : '' }}" name="angkatan_id" id="angkatan_id" required>
                    @foreach($angkatans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('angkatan_id') ? old('angkatan_id') : $mMasterSiswa->angkatan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jurusan') ? 'is-invalid' : '' }}" name="jurusan_id" id="jurusan_id" required>
                    @foreach($jurusans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('jurusan_id') ? old('jurusan_id') : $mMasterSiswa->jurusan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kelas') ? 'is-invalid' : '' }}" name="kelas_id" id="kelas_id" required>
                    @foreach($kelas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kelas_id') ? old('kelas_id') : $mMasterSiswa->kelas->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('kelas'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kelamin') ? 'is-invalid' : '' }}" name="kelamin_id" id="kelamin_id" required>
                    @foreach($kelamins as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kelamin_id') ? old('kelamin_id') : $mMasterSiswa->kelamin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id" required>
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $mMasterSiswa->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
